...
In this script, we will model changes in relative abundance as a function of environmental changes at each of the sampling sites (environmental changes are calculated as differences in values of a predictor between modern vs. historical time period)
...
trial <- as.polygons(lc_1848)

sz1848 <- cellSize(lc_1848, unit = "m")

area1848 <- zonal(sz1848, lc_1848, sum)

trial <- terra::extract(sz1848, vect(sites_sf))
...
b <- st_intersection(st_as_sf(trial), sites_sf)
c <- b %>%
  mutate(area = as.numeric(st_area(b)))
...
## dataframe for glmm
glmm_cc_change <- left_join(relAbun_wide, clim_change_glmm, by="historical_site_code") %>% as.data.frame()
