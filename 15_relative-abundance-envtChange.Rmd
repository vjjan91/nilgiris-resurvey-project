...
glmm_grassland <- glmm_grassland %>% 
  mutate(abundIndex = log10(`2021`/`1850`))

m1 <- glmmTMB(abundIndex ~ 
                shola_grassland  +t_mean_dry_2010_z +
                ppt_dry_2010_z + 
                (1|historical_site_code) +(1|scientific_name),
               data = glmm_grassland,
            ziformula = ~1)
summary(m1)
...
m2 <- glmmTMB(relAbunChange ~  shola_forest + t_mean_dry_2010_z +
                ppt_dry_2010_z + (1|scientific_name) +
                (1|historical_site_code),
               data = glmm_forest)
            #ziformula = ~1)
summary(m2)
...
m2 <- update(m1, .~. -plantations)
m3 <- update(m2, .~. -shola_grassland)
m4 <- update(m3, .~. -settlements)
m5 <- update(m4, .~. -shola_forest)
