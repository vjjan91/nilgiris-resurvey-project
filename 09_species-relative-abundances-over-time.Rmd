...
## calculating relative species abundances for the time period 1950-2000

# we remove modern site_code and get only distinct rows
unique_hist_1950_2000 <- hist_occ_sites_1950_2000[,c(1,3)] %>%
  group_by(scientific_name) %>%
  summarise("relAbund1950" = sum(`1950-2000`)) 

# apply dirichlet distribution
dat <- tibble::deframe(unique_hist_1950_2000) ## choosing 1950-2000 data

relAbun1950 <- dirch_stats(dat) %>%
  dplyr::select(species, bayes_mean) %>%
  rename(., bayes_mean_1950 = bayes_mean)
...
relAbun <- full_join(relAbun1850, relAbun1900, by = c("species")) %>%
  full_join(., relAbun1950, by = c("species")) %>%
...
fig_1850v2021_rel <- ggplot(data=relAbun,aes(x=bayes_mean_2021,y=bayes_mean_1950))    + geom_point() +
   scale_y_log10() +
  ...
  labs(y = "1850-1900 Relative abundance", 
   ...
   geom_smooth(method="lm", se=TRUE, fullrange=FALSE, level=0.95,linetype="solid") +  theme_bw(base_size=20) 
...
# we remove modern site_code and get only distinct rows
hist_1950_2000 <- hist_occ_sites_1950_2000[,c(1:3)] %>%
  distinct(.)

relSpAbund_1950_2000 <- c()

for(i in 1:length(unique(hist_1950_2000$historical_site_code))) {
  
  a <- hist_1950_2000 %>%
  filter(historical_site_code == unique(hist_1950_2000$historical_site_code)[i])

  b <- tibble::deframe(a[,c(1,3)]) ## choosing 1950-2000 data
  
  loc <- unique(hist_1950_2000$historical_site_code)[i] # what's the site_code?
...
Takeaways: No clear differences in relative abundances over time seen at the landscape level and site-level. Due to incomplete sampling, there are some sites that show a relatively higher abundance in earlier time periods when compared to modern occurrence. Unsure if the decrease seen between 1950 to 2000 is a real pattern/artifact of incomplete sampling. 
