...
```{r}
## ensure that NAs are removed from the stack object before running the model projections
terra::global(stack, fun = "isNA") # this tells us that two raster layers within the stack object have NA values
...
terra::writeRaster(gam_pred_avg, filename = "results/climate/gam_pred_valid_avg.tif")
```
...
We obtained Bioclimatic data from CHELSA. We do not upload the rasters to GitHub as they are very large in size and these can be downloaded from this link: https://envicloud.wsl.ch/#/?prefix=chelsa%2Fchelsa_V1
...
We obtain precipitation data from CHELSA, which can be accessed at this link here: https://envicloud.wsl.ch/#/?prefix=chelsa%2Fchelsa_V1
...
# remove id
survey_sites <- select(survey_sites, -c(site_name, historical_site_code, modern_landCover_type, ID))
