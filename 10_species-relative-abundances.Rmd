...
mod_occ <- read.csv("data/modern-occurrence-data.csv") %>%
  filter(!is.na(species_code)) %>% filter(common_name %in% hist_occ$common_name) %>%
...
![Historical vs. modern species relative abundance](figs/fig_historical_vs_modern_relAbun.png)
...
site_level_relAbun <- full_join(hist_pre1900,
  ...
  replace_na(list(`1850-1900` = 0, 
                  ...
                  `2021` = 0)) %>% 
  ungroup() %>%
  complete(common_name, historical_site_code, fill = list(`1850-1900` = 0, 
     `1900-1950` = 0,                                                     `2021` = 0))
