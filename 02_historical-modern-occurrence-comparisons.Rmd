...
# add elevation
hist_occ_elev <-  left_join(histOcc_spp, sites, by=c("historical_site_code"="historical_site_code")) %>%
  filter(!is.na(scientific_name)) %>%
  replace(is.na(.), 0) %>%
  mutate(Historic = case_when(histCount >= 1 ~ 1,TRUE ~ 0))
...
mod_occ_elev <-  left_join(modOcc_spp, sites, by=c("modern_site_code"="modern_site_code")) %>%
  filter(!is.na(scientific_name)) %>%
  replace(is.na(.), 0) %>%
  mutate(Modern = case_when(modCount >= 1 ~ 1,TRUE ~ 0))
```
...
hist_mod <- full_join(hist_occ_elev, mod_occ_elev, by = c("scientific_name","modern_site_code","historical_site_code","elev","site_name","geometry")) %>%
  replace(is.na(.), 0) %>%
...
for(i in 1:length(unique(hist_mod$scientific_name))) {
  ...
  data <- hist_mod[hist_mod$scientific_name==a,] %>%
      pivot_longer(cols=c(Historic, Modern), 
