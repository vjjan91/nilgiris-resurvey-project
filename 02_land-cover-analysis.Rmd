...
trial <-  st_read("data/spatial/1910-new-clipped-version.shp")

trial <- st_read("data/spatial/1910-nilgiris/1910.shp")
trial <- st_buffer(trial, dist = 0)

boundToClip <- st_buffer(st_union(nil1870), dist = 0)

trial2 <- st_intersection(trial, boundToClip)

# we decided to use the 1870 nilgiris raster only as the boundary to clip to 
# fix the names of land cover classes for 1910
# double check if certain rows are being duplicated??
# clip 1910 by 1870 boundary
# clip 1973, 1995, 2017 by 1870 boundary

# then land cover change detection


trial3 <- st_intersection(trial, contourNil)
...
# cast from MULTIPOLYGON to POLYGON to match nil2017 shapefile (not required @pratikunterwegs?)
...
```{r}
# Please note that for the sake of calculation of areas, we separate the nil1870 and kun 1870 files as they are of different resolutions and need to be resampled and rasterized
...
emptyRast1910 <- terra::rast(res = 30, xmin = 655224.34214709, xmax = 718286.685853398, ymin = 1230555.09211126, ymax = 1275965.51753469, crs = "+proj=utm +zone=43 +datum=WGS84 +units=m +no_defs")
...
areaRast1910 <- areaRast1910 %>%
  filter(!is.na(class)) %>%
  select(class, sumArea)

areaRast1910$class <- c(
  "Shola Forest", "Shola Grassland",
  "Agricultural Land", "Plantations (Tea & Timber)", "Unclassified Area",
  "Settlements", "Water bodies"
)
```
...
names(areaCalc) <- c("class", "1870", "1910","areaMt1973", "1973", "areaMt1995", "1995", "areaMt2017", "2017")
