...
nil1910 <- st_read("data/spatial/1910-nilgiris/1910.shp")

# note that there are four geometries empty and certain class names need to be fixed
# the CRS is a projected coordinate system (UTM43N)

# remove empty geometries
nil1910 <- nil1910[!st_is_empty(nil1910$geometry),]

# renaming column values for class column
names(nil1910)[2] <- "class"
nil1910$class <- recode(nil1910$class,
                      "cultivated areacultivated area" = "cultivated area",
                      "grassland" = "Shola Grassland",
                      "remaining" = "Unclassified area",
                      "reserve forest" = "Shola Forest")

# transform to UTM 43N
nil1910 <- st_transform(nil1910, 32643)

# clip to Nilgiris 1870 boundary
boundToClip <- st_buffer(st_union(nil1870), dist = 0)
nil1910 <- st_buffer(nil1910, dist = 0)
nil1910 <- st_intersection(nil1910, boundToClip)
```
...
nil1973 <- st_intersection(nil1973, boundToClip)
nil1995 <- st_intersection(nil1995, boundToClip)
nil2017 <- st_intersection(nil2017, boundToClip)
```
...
# nil1910 raster
vectnil1910 <- terra::vect(nil1910)
emptyRast1910 <- terra::rast(res = 30, xmin = 661012.738348484, xmax = 718286.685853398, ymin = 1240744.4111945, ymax = 1275965.51753469, crs = "+proj=utm +zone=43 +datum=WGS84 +units=m +no_defs")
rastnil1910 <- terra::rasterize(vectnil1910, emptyRast1910, "class")
sz1910 <- cellSize(rastnil1910, unit = "m")
areaRast1910 <- zonal(sz1910, rastnil1910, sum)
areaRast1910SqKm <- (areaRast1910$area / 1000000)
areaRast1910 <- cbind(areaRast1910, areaRast1910SqKm)
names(areaRast1910) <- c("class", "areaInMeters", "sumArea")

# Let's load the 1973, 1995 and 2017 series and rasterize them
vectnil1973 <- vect(nil1973)
emptyRast1973 <- terra::rast(res = 30, xmin = 661012.738348484, xmax = 718286.685853398, ymin = 1240744.4111945, ymax = 1275965.51753469, crs = "+proj=utm +zone=43 +datum=WGS84 +units=m +no_defs")
...
emptyRast1995 <- terra::rast(res = 30, xmin = 661012.738348484, xmax = 718286.685853398, ymin = 1240744.4111945, ymax = 1275965.51753469, crs = "+proj=utm +zone=43 +datum=WGS84 +units=m +no_defs")
...
emptyRast2017 <- terra::rast(res = 30, xmin = 661012.738348484, xmax = 718286.685853398, ymin = 1240744.4111945, ymax = 1275965.51753469, crs = "+proj=utm +zone=43 +datum=WGS84 +units=m +no_defs")
...
### Cleaning up the 1870 and 1910 maps
```{r}
# 1870 map
areaNil1870 <- areaNil1870 %>%
  filter(!is.na(class))
...
# cleaning up the 1910 raster
areaRast1910$class <- c(
  "Shola Forest", "Shola Grassland", "Unclassified Area", 
  "Agricultural Land", "Plantations (Tea & Timber)",
  "Settlements", "Water bodies"
)
```

### Calculating areas of different land cover classes across time periods
...
areaCalc <- purrr::reduce(list(
  areaNil1870, areaRast1910, areaRast1973, areaRast1995,
  areaRast2017
), dplyr::full_join, by = "class")

names(areaCalc) <- c("class", "areaMt1870", "1870", "areaMt1910",
                     "1910","areaMt1973", "1973", "areaMt1995", "1995",
                     "areaMt2017", "2017")

areaCalc <- areaCalc %>%
  select("class", "1870", "1910","1973", "1995", "2017") %>%
...
# to do:
# issues so far:
# coordinate system does not necessarily align for 1870 maps (nilgiris and kundah)
# issue with remaining area in 1910 map
# is it still ok for me to use st_intersection to ensure that it's approx. the same area that we are trying to calculate differences in area? 

# what is this script trying to do:
# a) calculate land cover area for different classes
# b) at a later step - extract lc class where resurveys were carried out? 
