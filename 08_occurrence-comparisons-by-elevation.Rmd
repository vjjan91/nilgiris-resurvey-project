...
Load the subset data for downstream analysis
```{r}
...
mod_pres_abs <-  left_join(mod_occ, sites, by=c("modern_site_code"="modern_site_code")) %>% filter(!is.na(scientific_name)) %>%
  replace(is.na(.), 0)  %>%
...
hist_mod <- full_join(hist_pres_abs, mod_pres_abs, by =               c("scientific_name","historical_site_code","modern_site_code","elev",
  "site_name","modern_landCover_type","geometry")) %>%
...
# for the creation of a separate plot where we compare
# data for each species across sites where they were detected in 1850
hist_mod <- hist_mod %>%
 filter(Y1850 == 1)
...
cairo_pdf(
  filename = "figs/elevation-historical-modern-occurrence.pdf",
  onefile = TRUE
)
plots
dev.off()
