...
We combine the data and filter for the southern Western Ghats as our area of interest based on the latitudinal range and write them to file.
...
**Note**: the use of `terra::deepcopy()` to make a copy of the elevation raster; this is to prevent the values of the elevation raster changing when the copy's values are re-assigned. See the help for `terra::deepcopy()` for more.
