...
This script analyzes land cover data between 1870 and 2017
```{r}
library(sf)
library(raster)
library(terra)
...
Load the contour for the Nilgiris to which all other shapefiles will be clipped to
```{r}
# load the 1400m contour
contourNil <- st_read("data/spatial/contour-1400m-nilgiris.shp")
```

Load all shapefiles across time periods:

Please note that the 1870 published maps were based on surveys carried out in 1848 across the landscape. 
```{r}
# list all shapefiles in the directory
nilgiris <- list.files("data/spatial/1870-nilgiris/", full.names = T,
                    recursive = T, pattern=".shp$")

# create vector files
cultivated_nil1870 <- st_read(nilgiris[1])
forest_nil1870 <- st_read(nilgiris[2])
grassland_nil1870 <- st_read(nilgiris[3])
plantations_nil1870 <- st_read(nilgiris[4])
roads_nil1870 <- st_read(nilgiris[5])
settlements_nil1870 <- st_read(nilgiris[6])
waterBodies_nil1870 <- st_read(nilgiris[7])
...
nil1870 <- rbind(cultivated_nil1870, forest_nil1870, grassland_nil1870,
...
Loading the Kundah 1870 shapefiles
```{r}
# load the Kundah 1870 shapefiles
kundah <- list.files("data/spatial/1870-kundah/", full.names = T,
                    recursive = T, pattern=".shp$")
...
nil1973 <- nil1973 %>%
  mutate(class = case_when(gridcode == 1 ~ "Shola Grassland",
...
Calculating areas of land cover types 
...
areaNil1870$class <- c("Shola Forest","Shola Grassland",
...
areaCalc <- purrr::reduce(list(areaNil1870, areaRast1973, areaRast1995,
                            areaRast2017), dplyr::full_join, by = 'class')
...
# make plot
fig_area <- ggplot(areaCalc, aes(x=class, y = Area, fill=class)) + geom_bar(stat="identity", position=position_dodge(), fill="#883107", alpha=0.9) +
...
ggsave(fig_area, filename = "figs/fig_totalArea_landCover.png", width=15, height=13,
...
percentArea <- areaCalc %>%
  group_by(Year) %>%
  mutate(totalArea = sum(Area,na.rm = T),
         percentArea = (Area/totalArea)*100)

# plot figure
fig_percent_area <- ggplot(percentArea, aes(x=class, y = percentArea, fill=class)) + geom_bar(stat="identity", position=position_dodge(), alpha=0.9) +
